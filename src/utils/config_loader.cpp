...
// ./app --exchange lmax --config .env/app.cfg --instance 2
...
namespace bluestone {
  namespace utils {
    constexpr std::string_view bluestone::utils::ConfigLoader::trim(
    ...
    bluestone::utils::ExchangeConfig bluestone::utils::ConfigLoader::load(
        int argc, char** argv) {
      ...
      while (std::getline(file, line)) {
        ...
        if (key == "host") {
          ...
          std::cout << "=== [ERROR]: bluestone::ConfigLoader::load(.. , ..) "
                       "wrong key="
                    << key << '\n';
        }
      }
