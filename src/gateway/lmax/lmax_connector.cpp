...
namespace bluestone {
LMAXConnector::LMAXConnector(const bluestone::ExchangeConfig& cfg) : cfg_(cfg) {
...
void LMAXConnector::subscribe_market_data(int req_id,
  ...
  // 1. MDReqID (Tag 262): Use the universal req_id passed from the Core Engine
  ...
  symbolGroup.set(FIX::SecurityIDSource(FIX::SecurityIDSource_EXCHANGE_SYMBOL));
  ...
  std::cout << "[LMAX] Sent Market Data Request (ID: " << req_id
            << ") for Instrument: " << symbol << std::endl;
}
...
void LMAXConnector::fromApp(const FIX::Message& message,
                            const FIX::SessionID& sessionID) {
  // CRITICAL: This routes incoming messages to the correct onMessage()
  // callback
