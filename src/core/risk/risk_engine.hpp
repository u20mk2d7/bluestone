...
The No-Atomic Rule: Look at current_position_. In standard software, tracking a
shared inventory requires std::atomic<int64_t> or a std::mutex. But because of
your architecture—pinning one Bot Instance to one Physical Core—no other thread
will ever touch this memory. This drops your memory access time from ~25ns down
to ~1ns (pure L1 cache read).

The [[nodiscard]] Attribute: In HFT, human error is the biggest liability. If a
Junior Quant writes risk_engine.evaluate_order(...); but forgets to write the if
statement to handle a rejection, LLVM Clang 22 will physically refuse to compile
the codebase.

The const Guarantee: Notice evaluate_order is marked const. It proves to the
compiler that evaluating risk does not mutate the engine's state, allowing LLVM
to aggressively cache the limits_ variables in the CPU registers.
*/
