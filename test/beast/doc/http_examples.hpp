...
namespace boost {
  namespace beast {
    namespace http {
      ...
      */
      template <class SyncStream, class DynamicBuffer, class Body,
                class Allocator>
      void send_expect_100_continue(SyncStream& stream, DynamicBuffer& buffer,
      ...
      */
      ...
      void receive_expect_100_continue(SyncStream& stream,
                                       DynamicBuffer& buffer, error_code& ec) {
        static_assert(is_sync_stream<SyncStream>::value,
                      "SyncStream requirements not met");
      ...
      */
      ...
      void send_cgi_response(SyncReadStream& input, SyncWriteStream& output,
                             error_code& ec) {
        ...
        do {
          ...
          auto bytes_transferred =
              input.read_some(boost::asio::buffer(buffer, sizeof(buffer)), ec);
       ...
       */
      template <class SyncStream, class DynamicBuffer>
      void do_server_head(SyncStream& stream, DynamicBuffer& buffer,
                          error_code& ec) {
        static_assert(is_sync_stream<SyncStream>::value,
                      "SyncStream requirements not met");
        static_assert(boost::asio::is_dynamic_buffer<DynamicBuffer>::value,
      ...
      */
      template <class SyncStream, class DynamicBuffer>
      response<empty_body> do_head_request(SyncStream& stream,
      ...
      */
      ...
      void relay(SyncWriteStream& output, SyncReadStream& input,
                 DynamicBuffer& buffer, error_code& ec, Transform&& transform) {
        static_assert(is_sync_write_stream<SyncWriteStream>::value,
                      "SyncWriteStream requirements not met");
      ...
      */
      ...
      void write_ostream(std::ostream& os,
      ...
      /** Read a message from a `std::istream`.
          ...
          This function attempts to parse a complete HTTP/1 message from the
         stream.
      ...
      */
      ...
      void read_istream(std::istream& is, basic_flat_buffer<Allocator>& buffer,
        ...
        do {
          ...
          if (is.rdbuf()->in_avail() > 0) {
            ...
            if (!is.eof()) {
              ...
              if (is.fail() && !is.eof()) {
                // We'll just re-use io_error since std::istream has no
                // error_code interface.
                ec = make_error_code(errc::io_error);
                return;
              }
        ...
        // Transfer ownership of the message container in the parser to the
        // caller.
        msg = p.release();
      }
      ...
      /** Handle a form POST request, choosing a body type depending on the
         Content-Type.
          ...
          the message body. Otherwise, a `dynamic_body` is used to store the
         message body. After the request is received, the handler will be
      ...
      */
      template <class SyncReadStream, class DynamicBuffer, class Handler>
      void do_form_request(SyncReadStream& stream, DynamicBuffer& buffer,
                           Handler&& handler) {
        ...
        switch (req0.get().method()) {
          case verb::post: {
            ...
            if (req0.get()[field::content_type] !=
      ...
      /*  This function reads a message using a fixed size buffer to hold
          portions of the body, and prints the body contents to a
         `std::ostream`.
      */
      template <bool isRequest, class SyncReadStream, class DynamicBuffer>
      void read_and_print_body(std::ostream& os, SyncReadStream& stream,
                               DynamicBuffer& buffer, error_code& ec) {
        parser<isRequest, buffer_body> p;
      ...
      */
      ...
      std::size_t read_large_response_body(
          SyncReadStream& stream, DynamicBuffer& buffer,
          message<isRequest, Body, basic_fields<Allocator>>& msg,
          std::size_t body_limit, error_code& ec) {
       ...
       */
      template <bool isRequest, class SyncReadStream, class DynamicBuffer>
      void print_chunked_body(std::ostream& os, SyncReadStream& stream,
                              DynamicBuffer& buffer, error_code& ec) {
        ...
        auto header_cb =
            [&](std::uint64_t
                    size,  // Size of the chunk, or zero for the last chunk
                string_view extensions,  // The raw chunk-extensions string.
                                         // Already validated.
                ...
                error_code& ec)        // We can set this to indicate an error
        {
          ...
          if (remain == body.size()) ec = error::end_of_chunk;
        ...
        while (!p.is_done()) {
          // Read as much as we can. When we reach the end of the chunk, the
          // chunk body callback will make the read return with the end_of_chunk
          // error.
          read(stream, buffer, p, ec);
          if (!ec)
            continue;
